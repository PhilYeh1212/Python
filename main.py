...
#GVxz5IXpDWRZAg8uSRggWwoTwWZE2jNitfevFmpc3wu --extel內部群
token = 'GVxz5IXpDWRZAg8uSRggWwoTwWZE2jNitfevFmpc3wu'
...
with open(r'C:\FS100.txt', 'r') as f:
    for i in range(0,100):
        ...
        sql1 = "%s%s.%s '%s'%s'%s'%s'%s'%s" % ('SELECT count(DAQ_Date) FROM FSLG_', sitecode[i], 'T1_head where DAQ_Date = (SELECT DATE_FORMAT(subdate(CURDATE(), 1),', '%Y/%m/%d', ')) and DAQ_Time >= ', StartTime, ' AND DAQ_Time < ', EndTime, ' GROUP BY daq_date')
    ...
    print(NightPower)
    if num <= 0:
        message = "Count<140_全案場正常"
        lineNotifyMessage(token, message)
    if num > 0:
        #message = '%s\n%s:%s%s:%s\n%s:%s' % ('\n缺資料機台', 'Date', date1, 'ID', DAS1, 'Count', Count1)
        message = ((('Count<140案場'+str(num)+'場'+'\n'), sorted(NightPower)))
        lineNotifyMessage(token, message)
